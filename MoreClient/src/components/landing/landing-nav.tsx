    ...
    >
      <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-5 sm:px-8">
        ...
        <div className="flex items-center gap-2.5 sm:gap-3">
          ...
          <a
            href="/welcome"
            className="hidden items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-foreground/80 transition-colors hover:text-foreground sm:flex"
          >
          ...
          <nav className="flex flex-col gap-3">
            {links.map((l) => (
              <a
                ...
                className="text-sm font-medium text-foreground/75 hover:text-accent dark:hover:text-[#00FFCC] py-1.5 transition-colors"
              >
                {l.label}
              </a>
            ))}
            <a
              href="/welcome"
              onClick={() => setOpen(false)}
              className="flex items-center gap-1.5 text-sm font-semibold text-foreground/70 py-1.5 hover:text-foreground"
            >
              <LogIn size={15} aria-hidden="true" />
              {t("navLogin")}
            </a>
            <Button href="/welcome" size="sm" className="mt-2 w-full bg-[#7B61FF] hover:bg-[#6848ff] text-white border-none">
              {t("navGetStarted")}
            </Button>
          </nav>
        </div>
