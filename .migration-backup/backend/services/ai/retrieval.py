...
def invalidate() -> None:
    """Drop the cached lexical indices (called on any KB write)."""
    global _indices
    _indices.clear()


def _get_index(tenant_key: str | None) -> dict:
    global _indices
    tenant = vectorstore._tenant_key(tenant_key)
    if tenant not in _indices:
        _indices[tenant] = _build_index(tenant)
    return _indices[tenant]


def _build_index(tenant_key: str | None) -> dict:
    rows = vectorstore.all_chunks(tenant_key)  # (id, text, metadata, embedding)
...
def hybrid_search(
    ...
    vec_hits = vectorstore.query(query_embedding, k=min(fetch, idx["n"]), tenant_key=tenant_key)
